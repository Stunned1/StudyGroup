...
export default function ScheduledLobbyForm({ userId }: { userId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("scheduled_lobbies").insert({
      host_id: userId,
    ...
    if (error) {
      ...
      router.push("/lobbies/scheduled");
      router.refresh();
    }
  }

  // Build the min value for the datetime-local input (now + 1 min)
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow p-6 flex flex-col gap-4">
      ...
      >
        {loading ? "Scheduling..." : "Schedule Lobby"}
      </button>
    </form>
  );
}
